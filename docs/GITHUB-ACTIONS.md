...
| `publish-openapi-azure-apim.yml`      | Reusable workflow: imports the versioned OpenAPI spec into Azure API Management. See [AZURE-APIM-PUBLISHING.md](./AZURE-APIM-PUBLISHING.md). The proposed multi-tenant (non-live + live) flow is described in [AZURE-APIM-RELEASE-FLOW.md](./AZURE-APIM-RELEASE-FLOW.md). | `workflow_call` (from `ci-draft.yml` / `ci-released.yml`) |
